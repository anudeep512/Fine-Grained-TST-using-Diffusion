...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            print(f"Step {self.step + self.resume_step + 1}/{self.learning_steps}: Time taken = {step_time:.4f} seconds")
            # print(self.eval_data is not None)
            # if self.step % self.log_interval == 0:
            #     logger.dumpkvs()
            # print(self.eval_data is not None)
            # print(self.step % self.eval_interval)
    ...
    def forward_only(self, batch, cond):
        with th.no_grad():
            ...
            for loss_name, loss_value in losses.items():
                if loss_name == 'loss':
                    print(f"Validation Loss: {loss_value.mean().item()}")
